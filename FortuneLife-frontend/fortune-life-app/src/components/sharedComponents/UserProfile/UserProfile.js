  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        setUserDto(response || {}); // Ensure userDto is not null
        setAddressDto(response?.addressDto || {}); // Ensure addressDto is not null
        setIsLoading(false); // Data has loaded, stop loading state
      } catch (error) {
