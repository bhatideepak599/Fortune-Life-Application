...
const ReplyToQuery = ({ query, setQuery, onClose }) => {
  ...
  const isSendDisabled = !response || response === query.answer;
    ...
    <div style={containerStyle}>
      {loading ? (
        ...
        <>
          ...
          <div>
            <textarea style={textAreaStyle} value={response} onChange={handleChange} rows="6" placeholder="Type your answer here..." />
            <div style={buttonGroupStyle}>
              <button onClick={handleSubmit} style={btnPrimaryStyle} onMouseOver={(e) => (e.currentTarget.style.backgroundColor = btnPrimaryHoverStyle.backgroundColor)} onMouseOut={(e) => (e.currentTarget.style.backgroundColor = btnPrimaryStyle.backgroundColor)} disabled={isSendDisabled}>
                Send
              </button>
            </div>
          </div>
