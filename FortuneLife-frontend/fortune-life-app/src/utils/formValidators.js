  ...
  validators: {
    minLowercase: {
      ...
      rule: (val, params, validator) =>
        validator.helpers.testRegex(val, /[a-z]/) && val.length >= params[0],
    },
    minUppercase: {
    ...
    minNumbers: {
      ...
      rule: (val, params, validator) =>
        validator.helpers.testRegex(val, /\d/) && val.length >= params[0],
    },
    minSymbols: {
      message: 'The :attribute must contain at least :minSymbols special character.',
      rule: (val, params, validator) =>
        validator.helpers.testRegex(val, /[@$!%*?&]/) && val.length >= params[0],
    },
    validUsername: {
      message:
        'The :attribute must be between 5 and 50 characters, include at least one lowercase letter, one number, and one special character (@, ., _, -).',
      rule: (val) =>
        /^(?=.*[a-z])(?=.*\d)(?=.*[@._-])[A-Za-z\d@._-]{5,50}$/.test(val),
    },
...
export const validatePassword = (password) => {
  const rules =
    'required|min:6|max:20|minLowercase:1|minUppercase:1|minNumbers:1|minSymbols:1';
